...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
      isScrolled 
        ? (isDarkMode ? 'bg-slate-950/85 backdrop-blur-md border-b border-cyan-900/40' : 'bg-white/90 backdrop-blur-md border-b border-cyan-100') 
        : 'bg-transparent'
    }`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-20">
            ...
            {navLinks.map((link) => (
              <a
                ...
                className={`px-3.5 py-2 rounded-lg text-sm font-medium transition-all duration-200 ${
                  activeSection === link.id ? 'text-cyan-400 bg-cyan-950/20' : 'text-slate-300 hover:text-cyan-300'
                }`}
              >
                {link.name}
              </a>
          ...
          <div className="hidden sm:flex items-center gap-3">
            <button onClick={toggleDarkMode} className="p-2.5 rounded-xl border border-slate-700 text-yellow-400 hover:border-yellow-400/50">
      ...
      {/* Izbornik za mobitel */}
      {mobileMenuOpen && (
        <div className={`lg:hidden ${isDarkMode ? 'bg-slate-950/95' : 'bg-white/95'} px-4 py-6 space-y-2`}>
          {navLinks.map((link) => (
            <a key={link.id} href={link.href} onClick={() => setMobileMenuOpen(false)} className="block px-4 py-2 text-slate-300 hover:text-cyan-400">
              {link.name}
            </a>
          ))}
        </div>
